...
2. Ports out: agregar `Decrypt` a `CredentialStore`; agregar `Create` a `AdministratorRepository`; agregar `FindByID`/`Delete` a `PendingEnrollmentRepository`; nuevos `TOTPVerifier` (separado de `TOTPSecretGenerator` por SRP), `AdministratorSessionRepository`, `SessionTokenGenerator`, `Transactor` (ajuste post-aprobación: `WithinTransaction(ctx, func(ctx) error) error`, para envolver `Administrator.Create` + `AdministratorSession.Save` en una sola transacción).
3. Port in: `VerifyAdministratorSetupUseCase`.
4. Usecase `VerifyAdministratorSetup`: rate limit → parsear `enrollment_id` → `FindByID` (nil = no encontrado) → `IsExpired` → `Decrypt` el secreto → `TOTPVerifier.Verify` (±1 período) → `ExistsActive` (chequeo temprano, fuera de transacción) → `NewAdministrator` → **dentro de `Transactor.WithinTransaction`**: `AdministratorRepository.Create` (con `password_hash` y el mismo ciphertext ya guardado en el pending enrollment, sin re-cifrar) → `SessionTokenGenerator.Generate` → `NewAdministratorSession` → `AdministratorSessionRepository.Save` — → fuera de la transacción, `PendingEnrollmentRepository.Delete`. Cualquier fallo en un paso previo no ejecuta los siguientes; cualquier error de infraestructura se propaga tal cual (mapea a 500 en el handler), salvo la violación del índice único de `email` en `Create` (ver Error handling impact).
5. Adapters de persistencia: agregar `EncryptedTOTPSecret []byte` a `model.Administrator` (columna nueva vía migración `AddColumn`, no `AutoMigrate`); nuevo `model.AdministratorSession` (`administrator_sessions`, con `token_hash` único); dos migraciones nuevas (`20260822_03`, `20260822_04`); repositorios `administrator.Create`, `pending_enrollment.{FindByID,Delete}`, nuevo `repository/administrator_session`; nuevo paquete `internal/adapter/db/postgres/txcontext` (helper `With`/`From` para propagar la `*gorm.DB` transaccional por `context.Context`, usado por `Create` y `Save` para participar de la misma transacción) y `internal/adapter/db/postgres/transactor.go` implementando `out.Transactor`.
...
Un sentinel nuevo: `ErrInvalidTotpEnrollmentVerification` (`0x401008V`, 400) — cubre `enrollment_id` inexistente, expirado, y `totp_code` incorrecto, sin distinguir la causa en la respuesta (mismo criterio ADR-008 que ya se aplicó a `ErrInvalidBootstrapToken`). Se reutiliza `ErrAdministratorAlreadyExists` (409) para la carrera de doble activación — **tanto el chequeo previo `ExistsActive` como, ahora, la violación del índice único de `email` detectada dentro de `AdministratorRepository.Create`** (`pgconn.PgError.Code == "23505"` mapeado explícitamente al mismo sentinel, en vez de propagarse como error opaco de infraestructura) — y el sentinel de rate limit ya existente (`auth.ErrSetupRateLimited`, 429). Documentado en `docs/errors/aaa-map.md`.
...
- `AdministratorRepository.Create` + `AdministratorSessionRepository.Save` corren dentro de una única transacción GORM (`out.Transactor.WithinTransaction`, ajuste pedido por el usuario tras la primera versión de este plan); `PendingEnrollmentRepository.Delete` queda deliberadamente fuera. Si el proceso falla después de confirmar esa transacción pero antes del `Delete`, queda un pending enrollment huérfano — inofensivo, porque `ExistsActive` ya es `true` y cualquier reintento de `/auth/setup/verify` u `/auth/setup` falla limpiamente con 409/400.
- Dos verificaciones casi simultáneas con el mismo código válido pueden pasar ambas el chequeo previo `ExistsActive` antes de que cualquiera entre a la transacción; el índice único de `email` en `administrators` sigue evitando una segunda fila, y ahora la request perdedora recibe **el mismo `409 ErrAdministratorAlreadyExists`** que la primera (en vez de un 500 genérico), porque `Create` traduce explícitamente la violación de constraint (`pgconn.PgError.Code == "23505"`) al sentinel existente.
