...
func TestLoadFromViperRejectsUnsafeSessionAndOrigins(t *testing.T) {
	...
	cases := map[string]func(*viper.Viper){
		...
		"public HTTP host":           func(v *viper.Viper) { v.Set("AKRITAS_PUBLIC_URL", "http://public.example.com") },
		"wildcard HTTP host":         func(v *viper.Viper) { v.Set("AKRITAS_PUBLIC_URL", "http://0.0.0.0:8080") },
...
func TestLoadFromViperAcceptsLocalHTTPPublicURL(t *testing.T) {
	t.Parallel()
	...
	for _, publicURL := range []string{"http://localhost:8080", "http://127.0.0.1:8080", "http://[::1]:8080"} {
		...
		t.Run(publicURL, func(t *testing.T) {
			...
			v := viper.New()
			v.Set("AKRITAS_DATABASE_URL", "postgres://user:password@db:5432/akritas")
			v.Set("AKRITAS_PUBLIC_URL", publicURL)
			v.Set("AKRITAS_MASTER_KEY", base64.StdEncoding.EncodeToString(make([]byte, 32)))
			v.Set("AKRITAS_PAGINATION_SECRET", "01234567890123456789012345678901")
			v.Set("AKRITAS_BOOTSTRAP_TOKEN", "01234567890123456789012345678901")
			...
			if configuration.PublicURL != publicURL {
				t.Fatalf("PublicURL = %q, want %q", configuration.PublicURL, publicURL)
			}
		})
	}
}
