...
// stale value.
func (r *repository) ConsumeTOTPPeriod(ctx context.Context, id uuid.UUID, expectedPasswordHash string, period int64) (bool, error) {
	result := txcontext.From(ctx, r.db).WithContext(ctx).Table("administrators").
		Where("id = ? AND password_hash = ? AND last_accepted_totp_period < ?", id, expectedPasswordHash, period).
