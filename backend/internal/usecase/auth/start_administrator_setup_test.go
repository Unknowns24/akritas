...
func (d *setupDeps) usecase() in.StartAdministratorSetupUseCase {
	return NewStartAdministratorSetupUseCase(
		...
		d.passwordHasher, d.bootstrapTokens, d.rateLimiter, d.clock,
	)
}
...
func TestStartAdministratorSetupHappyPath(t *testing.T) {
	...
	if saved.PasswordHash != deps.passwordHasher.hash {
		t.Fatal("saved enrollment must carry the hashed password")
	}
	wantCiphertext := "encrypted:" + deps.totpGenerator.secret.Base32Key
	if string(saved.EncryptedTOTPSecret) != wantCiphertext {
		t.Fatalf("saved enrollment must carry the ciphertext, got %q, want %q", saved.EncryptedTOTPSecret, wantCiphertext)
	}
...
func TestStartAdministratorSetupRateLimited(t *testing.T) {
	...
	if _, err := uc.Execute(context.Background(), validSetupInput()); !errors.Is(err, ErrSetupRateLimited) {
...
func TestStartAdministratorSetupDownstreamErrorsPropagateAndDoNotPersist(t *testing.T) {
	...
	for name, arrange := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			if deps.pendingEnrollments.saved != nil {
